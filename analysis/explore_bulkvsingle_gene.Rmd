...
prop_cell_function= function(c){
  return(dim(molecules_with_nonzero_all[molecules_with_nonzero_all$non_zero_cells_all==c,])[1])
}
...
plot(prop_cells/864, ylab="Proportion of genes detected per number of cels given gene observed in sigle cell or pulk ")


# given a gene is observed in single cell or bulk ( genes), we condition on the 
# number of cells (ranging 0 - 864 cells) and look at the proportion of cells
# that a gene is detected in single cell 


```

```{r}
prop_cell_function_SConly= function(c){
  return(dim(molecules_with_nonzero[molecules_with_nonzero$non_zero_cells==c,])[1])
}
...
for (i in seq(1:864)){
  num= prop_cell_function_SConly(i)
  prop_cells_SC=c(prop_cells_SC, num)
}
...
#condition on only single cell, 424 in 1 cell, 197 in 2 cells, number of genes found in that cell number  
```
