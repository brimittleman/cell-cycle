  ...
  do
    ...
    do
      ...
      do
        ID=`printf "%s.%s.%s%02d\n" $IND $REP $ROW $COL`
        echo $ID
        echo "samtools merge bam-combined/$ID.trim.sickle.sorted.combined.bam bam-processed/$ID*trim.sickle.sorted.bam" > tmp-$ID.sh
        sbatch tmp-$ID.sh
      done
    done
  done
...
```
Files are now in bam-combined. There are 72 files.You must specify at least 2 input files.  

The slurm-out files have the following information.  
Usage:   samtools merge [-nurlf] [-h inh.sam] [-b <bamlist.fofn>] <out.bam> <in1.bam> <in2.bam> [<in3.bam> ... <inN.
bam>]
...
Options: -n       sort by read names
         ...
         -b FILE  list of input BAM filenames, one per line [nul
